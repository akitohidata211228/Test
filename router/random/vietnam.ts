...
export default async function cecanvietnamHandler(
  req: Request,
  res: Response
) {
  try {
    const LIST_URL =
      "https://raw.githubusercontent.com/siputzx/Databasee/refs/heads/main/cecan/vietnam.json"
    ...
    console.error("Error cecan vietnam:", error)
    res.status(500).json({
      status: false,
