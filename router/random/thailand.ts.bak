...
export default async function cecanthailandHandler(
  req: Request,
  res: Response
) {
  try {
    const LIST_URL =
      "https://raw.githubusercontent.com/siputzx/Databasee/refs/heads/main/cecan/thailand.json"
    ...
    console.error("Error cecan thailand:", error)
    res.status(500).json({
      status: false,
