...
export default async function cecanjepangHandler(
  req: Request,
  res: Response
) {
  try {
    const LIST_URL =
      "https://raw.githubusercontent.com/siputzx/Databasee/refs/heads/main/cecan/korea.json"
    ...
    console.error("Error cecan jepang:", error)
    res.status(500).json({
      status: false,
