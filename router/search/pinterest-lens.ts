...
async function searchPinterestFromUrl(imageUrl: string) {
  ...
  /*
    Token & cookie Pinterest di source aslinya hardcode. Itu kredensial, jadi
    dipindah ke env: isi PINTEREST_TOKEN (dan PINTEREST_COOKIE kalau perlu)
    biar endpoint ini hidup.
  */
  if (!process.env.PINTEREST_TOKEN) {
    throw new Error("PINTEREST_TOKEN belum diisi di environment server")
  }
